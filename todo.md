# Experimental Campaign TODO

- [x] Auditar dados, código de treinamento, protocolos, artefatos existentes e recursos computacionais.
- [ ] Localizar e validar os diretórios extraídos autorizados de FakeAVCeleb e AV-LipSync-TIMIT.
- [ ] Pesquisar fontes oficiais e condições de acesso para obter os dois datasets ou alternativas publicamente permitidas.
- [ ] Obter somente os dados licenciados ou publicamente acessíveis e registrar quaisquer etapas que exijam aprovação do mantenedor.
- [ ] Preencher, com os dados acadêmicos fornecidos e o aceite pessoal do usuário, os formulários oficiais de FakeAVCeleb e LipSyncTIMIT.
- [ ] Submeter os formulários somente após confirmação explícita do usuário e registrar as solicitações realizadas.
- [x] Registrar a solicitação FakeAVCeleb enviada e manter a campanha oficial bloqueada até a aprovação dos mantenedores.
- [x] Implementar e validar o protocolo reprodutível, os scripts de ablação e os comparadores sem declarar métricas de benchmark antes do acesso aos dados.
- [x] Definir splits fixos, sementes, métricas, critérios de parada e comparadores multimodais.
- [ ] Preparar FakeAVCeleb e AV-LipSync-TIMIT para treinamento e avaliação reprodutíveis.
- [ ] Executar treinamento completo do SyncTrace e registrar métricas por base e semente.
- [ ] Executar ablações de severidade, Mamba--Attention e Spatiotemporal Attribution Engine.
- [ ] Comparar qualidade, latência, parâmetros, FLOPs e memória contra baselines multimodais.
- [ ] Atualizar tabelas, figuras e discussão do manuscrito com resultados verificados.
- [ ] Executar a suíte de validação e enviar código, resultados e manuscrito atualizados ao repositório remoto.
